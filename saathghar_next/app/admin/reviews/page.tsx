...
import { handleGetAdminReviews, handleDeleteAdminReview } from "@/lib/actions/admin-action";
...
export default function AdminReviewsPage() {
  ...
  const confirmDelete = async () => {
    const res = await handleDeleteAdminReview(deleteConfirm.id);
    if (res.success) {
  ...
  return (
    <div className="py-8 px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-extrabold text-slate-800 dark:text-white">Reviews</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage all user reviews.</p>
        </div>
                  ...
                  {reviews.map((r: any) => (
                    <tr key={r._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="py-4 px-6 font-semibold">
                        {r.reviewer?.fullName || r.reviewer?.username || "Unknown"}
                      </td>
                      <td className="py-4 px-6 text-slate-500 dark:text-slate-400">
                        {r.reviewee?.fullName || r.reviewee?.username || "Unknown"}
                      </td>
                        ...
                        <button
                          onClick={() =>
                            setDeleteConfirm({
                              open: true,
                              id: r._id,
                              name: `review by ${r.reviewer?.fullName || "user"}`,
                            })
                          }
